...
class ViTPoolModelOutput:
    ...
    sig_probabilities: torch.FloatTensor = None
    softmax_probabilities: torch.FloatTensor = None
...
class ViTPoolClassifier(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> ViTPoolModelOutput:
        ...
        sig_probs = torch.stack(
            [self.sigmoid(classifier(pool_op)) for classifier in self.classifiers],
            dim=1,
        )
        sig_probs = torch.mean(sig_probs, dim=1)

        softmax_probs = torch.stack(
            [self.softmax(classifier(pool_op)) for classifier in self.classifiers],
            dim=1,
        )
        softmax_probs = torch.mean(softmax_probs, dim=1)
