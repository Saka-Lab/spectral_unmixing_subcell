...
@torch.no_grad()
def run_model(model, cell_crop, device):
    ...
    sig_probabilities = output.sig_probabilities[0].cpu().numpy()
    softmax_probabilities = output.softmax_probabilities[0].cpu().numpy()
    embedding = output.pool_op[0].cpu().numpy()

    return np.array(embedding), np.array(sig_probabilities), np.array(softmax_probabilities)
